...
export class CartService {
  ...
  async getMyCarts(memberId: string) {
    ...
    if (!cart) {
      throw new NotFoundException(`no cart found with member id ${memberId}`)
    }
  ...
  async addToCart(
    ...
    const {
      ...
      _id,
      images,
    } = await this.products.getProductById(product.id)
  ...
  async updateCartProductQty(
    memberId: string,
    cartItem: {id: string; qty: number; size: string},
  ) {
    const cart = await this.carts.findOneAndUpdate(
      {
        memberId,
        'products.id': cartItem.id,
        'products.size': cartItem.size.trim(),
      },
    ...
    if (!cart) {
      throw new NotFoundException(
        `either product doesn't exist in cart or cart not exist with member id "${memberId}"`,
      )
    }
