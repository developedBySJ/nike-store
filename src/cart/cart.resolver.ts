...
export class CartResolver {
  ...
  async addToCart(
    ...
    @Args({ name: 'CartProductInput', type: () => CartProductInput }) product: AddToCartDto
  ) {
  ...
  async updateCartProductQty(
    @Args({ name: "UpdateCartProductQtyInput", type: () => UpdateCartProductQtyInput }) product: { id: string, qty: number, size: string },
    @CurrentUser() user: JwtPayload) {
