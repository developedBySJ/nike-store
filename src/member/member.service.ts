...
export class MemberService {
  ...
  async getAll({ limit, page, sortBy }: IMemberFilter) {
    ...
    if (!res.length) { throw new NotFoundException("no member found"); }
  ...
  async getById(id: string) {
    ...
    if (!res) { throw new NotFoundException(`member not found with id "${id}"`) }
  ...
  async getByEmail(email: string) {
    ...
    if (!res) { throw new NotFoundException(`member not found with email "${email}"`) }
  ...
  async updateMember(id: string, data: IUpdateMemberData, viewer: JwtPayload) {
    ...
    if ((String(viewer.id) === String(res._id) || viewer.isAdmin)) {
      ...
      const { address, avatar, currPassword, newPassword, dateOfBirth, firstName, isAdmin, lastName } = data;

      let password: string | undefined;

      if (currPassword && newPassword) {

        const isCorrectPassword = (await this.authService.isCorrectPassword(currPassword, res.password))
      ...
      if (avatar) {
        ...
        if (Array.isArray(avatar)) {
          ...
          avatarUrl = (await Cloudinary.upload(imgArr, CLOUDINARY_API.folder.members)).secure_url

        } else {

          avatarUrl = (await Cloudinary.upload(avatar.file, CLOUDINARY_API.folder.members)).secure_url
      ...
      Object.keys(updateObj).forEach((key: string): void => { updateObj[key] === undefined && delete updateObj[key] })

      res.set(updateObj)

      if (res.isModified("firstName lastName avatar")) {

        await this.reviews.updateMany({ "member.id": res._id }, {
          $set: {
