...
import { BadRequestException, ForbiddenException, forwardRef, Inject, UseGuards } from '@nestjs/common'
import { Args, Resolver, Query, ID, Mutation, ResolveField, Parent } from '@nestjs/graphql'
...
import { CurrentUser, CurrentUserCookie } from 'src/lib/decorators/CurrentUser.decorator'
...
export class MemberResolver {
  constructor(
    ...
    @Inject(forwardRef(() => FavouriteService)) private favouriteService: FavouriteService
  ) { }
  ...
  async getMembers(
    @Args({ name: 'MemberFilter', type: () => MemberFilter }) memberFilter: MemberFilterDto
  ) {
  ...
  async getMemberById(
    ...
    if (!isValidObjectId(id)) { throw new BadRequestException("invalid member id") }
  ...
  async whoAmI(
    @CurrentUserCookie() viewer: string,
  ) {
    ...
    const { _id, firstName, lastName, email, isAdmin, avatar } = await this.memberService.getById(user.id)

    return { id: _id, didRequest: true, firstName, lastName, email, isAdmin, avatar }
  ...
  @UseGuards(JwtAuthGuard)
  async updateMember(
    @Args({ name: 'UpdateUserInput', type: () => UpdateMemberInput }) updateMemberData: any,
    @CurrentUser() user: JwtPayload
  ) {
