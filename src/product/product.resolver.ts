...
export class ProductResolver {
  ...
  async getProducts(
    @Args({ name: "GetProductInput", type: () => ProductFilter, nullable: true }) filter: ProductFilterDto) {
  ...
  async getProductById(@Args({ name: 'id', type: () => ID }) id: string) {
    ...
    if (!isMongoId(id)) { throw new BadRequestException("invalid product id") }
  ...
  async createProduct(
    ...
    try {
      ...
      await validateOrReject(createProduct, { validationError: { target: false, value: true } })
  ...
  async updateProduct() { return "Not Implemented Yet" }
