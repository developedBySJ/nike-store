...
export class ProductService {
  ...
  async getProducts(filter: IProductFilter | undefined) {
    ...
    const { sortBy, limit, page, priceRange, ratings, searchQuery: q, ...other } = filter || {};
    const skip = limit * (page - 1)

    const category = other.category === "clothing" ?
      { category: { $in: ["hoodies", "tshirt", "jersys"] } }
      : other.category === "accessories" ?
        { category: { $in: ["cap", "bags"] } }
        : {}

    const searchQuery = q?.length > 0 ? {
      $or: [
    ...
    let productsQuery = this.products.find(query).limit(Math.min(limit || 8, 50)).skip(skip || 0)
  ...
  async createProduct(createProduct: ICreateProduct) {
    ...
    const images = productImg instanceof Array ?
      await Promise.all(productImg.map(async img => {

        const fileUpload = await img.promise

        return (await Cloudinary.upload(fileUpload, FOLDER.products)).secure_url
