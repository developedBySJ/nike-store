...
export class FavouriteService {
  ...
  async getMyFavourites(memberId: string) {
    ...
    if (!favourite) {
      throw new NotFoundException(
        `no favourites found with member id ${memberId}`,
      )
    }
  ...
  async addToFavourites(memberId: string, favouriteProductId: string) {
    ...
    if (!product) {
      throw new NotFoundException(`no product with member id ${memberId}`)
    }
    ...
    if (!favourite) {
      throw new NotFoundException(`no favourites with member id ${memberId}`)
    }
