...
export class ReviewResolver {
  ...
  @Query(() => [ReviewType])
  async getReviews(
    @Args({name: 'GetReviewsInput', type: () => ReviewsFilter})
    getReviewInput: GetReviewsDto,
  ) {
    if (!isMongoId(getReviewInput.productId)) {
  ...
  async getMyReviews(
    @Args({name: 'ReviewsFilter', type: () => ReviewsFilter, nullable: true})
    reviewsFilter: GetReviewsDto,
  ...
  @UseGuards(JwtAuthGuard)
  async createReview(
    @Args({name: 'CreateReviewInput', type: () => CreateReviewInput})
    createReviewInput: CreateReviewDto,
    @CurrentUser() user: JwtPayload,
  ) {
  ...
  @UseGuards(JwtAuthGuard)
  async updateReview(
    @Args({name: 'UpdateReviewInput', type: () => UpdateReviewInput})
    {reviewId, ...data}: UpdateReviewDto,
    @CurrentUser() user: JwtPayload,
  ) {
