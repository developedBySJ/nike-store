...
import { ICreateReview, IPreSave, IReviewFilter, IUpdateReview } from './review.type'
...
export class ReviewService {
  ...
  async getReviews({ productId, limit = 8, page = 0, sortBy }: IReviewFilter) {
    ...
    return this.reviews.find({ productId: productId })
      .limit(limit)
      .skip(skip)
      .sort(sort)
  ...
  async getReviewByUser(user: JwtPayload, { limit = 8, productId, page = 1, sortBy }: IReviewFilter) {
    ...
    return await this.reviews.find({ "member.id": user.id, ...productId && { productId } })
      .limit(limit)
      .skip(skip)
      .sort(sort)
  ...
  async updateReview({ viewer, reviewId, data: { comment, rating } }: IUpdateReview) {

    const review = await this.reviews.findById(reviewId)

    if (!review) { throw new NotFoundException(`no review found with id "${reviewId}"`) }
    ...
    if ((String(viewer.id) === String(review.member.id) || viewer.isAdmin)) {
      ...
      // @ts-ignore
      await this._preSave({ isNew: false, prevRating: review._prevRating || 0, productId: review.productId, rating })
  ...
  async deleteReview({ viewer, reviewId }: { viewer: JwtPayload, reviewId: string }) {
    ...
    if ((String(viewer.id) === String(review.member.id) || viewer.isAdmin)) {
      ...
      this._preRemove({ id: review.id, productId: review.productId, rating: review.rating })
  ...
  private async _preRemove({ productId, rating, id }) {
    ...
    const newRating = Number(((ratingSum - rating) / product.numOfReviews).toFixed(2));

    product.ratings = newRating;
    product.reviews = product.reviews.filter(prodReview => String(prodReview.id) !== String(id))
  ...
  private async _preSave({ productId, isNew, prevRating, rating }: IPreSave) {
    ...
    if (product) {
      ...
      if (isNew) { product.numOfReviews++; }

      // @ts-ignore
      const newRating = Number(((ratingSum + rating - prevRating) / product.numOfReviews).toFixed(2));
